...
const ListItems: React.FC<ListItemsProps> = ({ section }) => {
    ...
    <div className="flex flex-col">
      ...
      > 
        <div className="flex items-start gap-2">
          ...
          <p className="font-roboto tracking-wide text-[#A644CB] text-lg  font-bold transition-all duration-300 ease-in-out">{section.title}</p>
        </div>
        <div
      ...
      {isOpen && (
        <ul className="  pl-3 mt-3 ml-3 transition-all duration-300 ease-in-out">
          ...
          {section.items.map((item, index) => (
            <li key={index} className="p-1 border-[#A644CB] border-solid hover:border-b-2 rounded-xl">
              <Link className="flex items-start mb-2" href={item.route}>
